            ...
            <tbody>
              {workList.map((work, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{work.name}</td>
                  <td>{work.published ? <TextFormat type="date" value={work.published} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>
                    {work.responsibles
                      ? work.responsibles.map((val, j) => (
                          <span key={j}>
                            <Link to={`/person/${val.id}`}>{val.name}</Link>
                            {j === work.responsibles.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>
                    {work.sponsors
                      ? work.sponsors.map((val, j) => (
                          <span key={j}>
                            <Link to={`/institution/${val.id}`}>{val.name}</Link>
                            {j === work.sponsors.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>
                    {work.roles
                      ? work.roles.map((val, j) => (
                          <span key={j}>
                            <Link to={`/role-work/${val.id}`}>{val.name}</Link>
                            {j === work.roles.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
