...
public class InstitutionRepositoryWithBagRelationshipsImpl implements InstitutionRepositoryWithBagRelationships {
    ...
    public Optional<Institution> fetchBagRelationships(Optional<Institution> institution) {
        return institution.map(this::fetchRoles);
    }
    ...
    public List<Institution> fetchBagRelationships(List<Institution> institutions) {
        return Optional.of(institutions).map(this::fetchRoles).orElse(Collections.emptyList());
    }

    Institution fetchRoles(Institution result) {
        return entityManager
            .createQuery(
                "select institution from Institution institution left join fetch institution.roles where institution is :institution",
                Institution.class
            )
    ...
    List<Institution> fetchRoles(List<Institution> institutions) {
        ...
        List<Institution> result = entityManager
            .createQuery(
                "select distinct institution from Institution institution left join fetch institution.roles where institution in :institutions",
                Institution.class
            )
