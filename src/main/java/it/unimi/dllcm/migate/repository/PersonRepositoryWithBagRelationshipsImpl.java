...
public class PersonRepositoryWithBagRelationshipsImpl implements PersonRepositoryWithBagRelationships {
    ...
    public Optional<Person> fetchBagRelationships(Optional<Person> person) {
        return person.map(this::fetchJobs).map(this::fetchResponsibilities);
    }
    ...
    public List<Person> fetchBagRelationships(List<Person> people) {
        return Optional.of(people).map(this::fetchJobs).map(this::fetchResponsibilities).orElse(Collections.emptyList());
    }

    Person fetchJobs(Person result) {
        return entityManager
            .createQuery("select person from Person person left join fetch person.jobs where person is :person", Person.class)
            .setParameter("person", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Person> fetchJobs(List<Person> people) {
        ...
        List<Person> result = entityManager
            .createQuery("select distinct person from Person person left join fetch person.jobs where person in :people", Person.class)
