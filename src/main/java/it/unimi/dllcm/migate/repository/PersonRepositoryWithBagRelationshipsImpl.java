...
public class PersonRepositoryWithBagRelationshipsImpl implements PersonRepositoryWithBagRelationships {
    ...
    public Optional<Person> fetchBagRelationships(Optional<Person> person) {
        return person.map(this::fetchRoles).map(this::fetchRoles);
    }
    ...
    public List<Person> fetchBagRelationships(List<Person> people) {
        return Optional.of(people).map(this::fetchRoles).map(this::fetchRoles).orElse(Collections.emptyList());
    }

    Person fetchRoles(Person result) {
        return entityManager
            .createQuery("select person from Person person left join fetch person.roles where person is :person", Person.class)
            .setParameter("person", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Person> fetchRoles(List<Person> people) {
        ...
        List<Person> result = entityManager
            .createQuery("select distinct person from Person person left join fetch person.roles where person in :people", Person.class)
